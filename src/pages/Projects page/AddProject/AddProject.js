...
import { addProject , getAllEmployeeBeneficiaries} from './addProject-request';
...
function AddProject (props) {
    ...
    useEffect(() => {
        getAllEmployeeBeneficiaries().then(res => {
            setGetLoading(false);
            if(res.ok === true){
    ...
    const handleSubmit = async (e) => {
        ...
        //request
        setLoading(true);
        const res = await addProject(name , startDate , endDate , image , target , location
                                        , type , categories , projectGoals , projectDescription, projectEmployees , projectBeneficiaries);
        setLoading(false);
        if(res.ok === true){
            props.showMessage(true , 'success' , 'تم إضافة المشروع');
    ...
    return (
        <div className='add-project'>
            <DashboardPageHeader 
                mainTitle='إضافة مشروع'
                subTitle='يمكن من خلال هذا القسم أضافة مشروع الى مشاريع الجمعية .'
                listItem={['إضافة مشروع']}
            />
            <div className='add-project-content'>
                ...
                {!getLoading && !getError && <form dir='rtl' onSubmit={handleSubmit}>
                    ...
                    <div className="section6">
                            ...
                            <div className="employees">
                                {
                                    jobStates.map((value , i) => (
                                        <div key={value.toString()}>
                                            {(projectEmployees.findIndex(e => e.jobState === value) !== -1 || 
                                            (value === 'موظف' && projectEmployees.findIndex(e => e.jobState === 'متطوع') !== -1 )) ? 
                        ...
                        <div className="part2">
                            ...
                            <div className="beneficiaries">
                                {projectBeneficiaries.length !== 0 ? <div className='title'>كل المستفيدين</div> : <></>}
                                {
                    ...
                    {employeesPopUp && 
                        <div className = "employees-pop-up">
                            <PopUpList
                                list={employeesList}
                    ...
                    {beneficiariesPopUp && 
                        <div className = "beneficiaries-pop-up">
                            <PopUpList
                                list={beneficiariesList}
                                sortBar={beneficiariesSortBar}
                                show={setBeneficiariesPopUp}
                                value={projectBeneficiaries}
                                onChange={setProjectBeneficiaries}
                                handleSearch={handleBeneficiraiesSerach}
                                className = "beneficiaries-pop-up"
                            />
                        </div>
                    }
                </form>}
