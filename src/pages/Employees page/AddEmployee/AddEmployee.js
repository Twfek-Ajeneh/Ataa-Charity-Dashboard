...
// import add employee request
import { addEmployee } from './AddEmployee-request';
...
function AddEmployees (props) {
    ...
    const [birthdate , setBirthday] = useState(dateFormat(new Date()));
    ...
    const handleSubmit = async (e) => {
        ...
        //request
        setLoading(true);
        const res = await addEmployee(name , idNumber , email , birthdate 
                                    , phoneNumber , image , gender , category 
                                    , jobState , state , region , location , employeeDetails);
        setLoading(false);
        if(res.ok === true){
            props.showMessage(true , 'success' , 'تم إضافة الموظف');
    ...
    return (
        <div className='add-employee'>
            <DashboardPageHeader 
                mainTitle='إضافة موظف'
                subTitle='في هذا القسم يمكنك إضافة موظف الى الجمعية'
                listItem={[
                    'إضافة موظف'
                ]}
            />
